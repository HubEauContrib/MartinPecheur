...
| 2026-09-22 | **Regroupement des marqueurs par zone administrative** (arbitrage du commanditaire, validé en deux sections : comportement puis architecture) → [`ADR-015`](adr/ADR-015-regroupement-par-zone-administrative.md). Sous le zoom `flutter_map` **7**, une pastille par **région** ; de 7 à moins de **9**, une par **département** ; à partir de 9, marqueurs individuels (`F2c` inchangée). Barycentre calculé, compte, symbole **existant** de l'état le plus sévère (`BR-009`) ; compte seul sur l'échelle débit ; sélection → zoom sur l'emprise des membres, aucune fiche ; préchargement au seul niveau individuel ; un point sans région reste individuel. Amende `04-ui.md § 4`. Lot **4 bis** du plan T1 (`Z1` → `Z4`), entre `H2` et `K1` ; `Z1` (ADR, amendement) fait le jour même. **39 tâches actives** · 🔄 décidé, pas encore codé |
...
|---|---|---|
| **Porte de spike** | Fond IGN affiché (`F1`), exécutable Windows autonome (`F3`) | ✅ franchie sur Windows (exécution 2026-09-09, arbitrage 2026-09-12). `F2` (4 150 marqueurs clusterisés) **non tranchée par la mesure** ; l'approche est **tranchée par [`ADR-015`](adr/ADR-015-regroupement-par-zone-administrative.md) pour les zooms < 9** (regroupement par région puis par département, arbitrage du 2026-09-22), `F2c` au-delà — 🔄 décidé, pas encore codé. `NFR-01` reste à mesurer (`X3`) |
...
| **T1 — fiche station, écoulement ONDE, avertissements** | Carte interactive (tap, fiche), fiches station et ONDE, **modal**, ~~**bandeau, encart daté**~~ → **contrôle « ⚠ Avertissement »** sur la carte et chaque fiche (2026-09-23, point 41) — encart renforcé (`BR-013`) en T2 | 🔄 **en cours.** **Lots 1 (`D1`→`D8`), 2 (`V1`→`V4`) et 3 (`U1`→`U6`) clos** le 2026-09-14 ; ✅ **lot 3 constaté à l'écran sur Windows le 2026-09-22** (point 36). **Au 2026-09-23, lot 4 terminé** : `W1` (`6b9e9ed`), `W2` (`408d47e`), `W2b` (`42115ae`), `W3` (`f7c37ca`), `W3b` (`2fd2d1b`), `H1` (`d62a35d`), `W4` (`8e5780b`), `W5` (`40e0efd`), puis **`W3c`** (`80a2d4b`, point 41, remplace `W3`, `W3b` et l'encart de `W4`), ✅ **constaté à l'écran le 2026-09-23 — lot 4 clos** ; `H2` faite (`0cc8437`) ; `Z1` faite (`0a3b594`). **PR #12 fusionnée sur `dev`** (`5f02d3e`), **908 tests verts**. **Lot 4 bis clos le 2026-09-23** (`Z1`→`Z4`, 998 tests, `Z4` constatée à l'écran) ; `K1` faite (1 026 tests) ; `K2` et `K3` codées (1 077 tests, constat d'écran dû). Restent `K1`→`K3`, les lots 6 et 7 |
...
🚨 **Rien de tout cela n'est un produit.** `CLAUDE.md` interdit toute mise en production tant que les
quatre avertissements obligatoires (`BR-012`, `BR-013`) ne sont pas tous posés. **Au 2026-09-23, trois sont
posés par le lot 4 de T1** : le modal (`W2`, `W2b`), puis, pour la carte et les fiches, un seul contrôle
« ⚠ Avertissement » (`W3c`, point 41 — il remplace le bandeau et l'encart daté ; à committer, jamais
constaté à l'écran). L'encart renforcé de `BR-013` est reporté en **T2** (arbitrage du 2026-09-22) : aucune
mise en production n'a donc lieu avant T2.
...
| Avertissements | ✅ Les 4 emplacements **spécifiés**, textes rédigés. **Posés dans le code (lot 4 de T1) :** le modal bloquant du premier lancement (`W2`, `W2b`, constaté à l'écran) ; depuis l'arbitrage du 2026-09-23 (point 41, `W3c`), **un seul contrôle « ⚠ Avertissement »** — sur la carte en haut à droite au-dessus de la légende, et en tête de chaque fiche avant la valeur ou la catégorie — qui ouvre en lecture seule la fenêtre « Des informations, pas une autorisation » (texte du modal initial), complétée **sous** ce texte, sur une fiche datée, par sa phrase propre (`lib/features/shared/warning_link.dart`). Il **remplace** le bandeau de carte (`W3`), son menu de rappel (`W3b`) et l'encart daté de fiche (`W4`, le widget ; le texte `sheetWarningText` reste). ✅ `W3c` **constaté à l'écran sur Windows le 2026-09-23** par le commanditaire. L'encart renforcé de `BR-013` est **reporté en T2** (arbitrage du 2026-09-22), son texte écrit et figé (`W5`) |
...
| Lots 4 à 7 | Avertissements (`W1`-`W3`, `W2b`, `W3b`, **`W3c`** — contrôle « ⚠ Avertissement », 2026-09-23, point 41 —, **`H1`** formateur de date en heure locale, `W4`, `W5`), **lot 4 bis** regroupement par zone administrative (`Z1`-`Z4`, `ADR-015`, entre `H2` et `K1` — `Z1` fait le 2026-09-22), clavier/souris (**`H2`** décisions de la carte rendues au ViewModel, `K1`-`K3`), documentation (`X1`-`X5`, dont la **purge React Native** `X5`), porte `0.2.0` (`P1`, `P2`) — `H1` et `H2` ajoutées le 2026-09-22 | ✅ **lot 4 terminé le 2026-09-23** (commits : voir la ligne T1 de « Où on en est » ; `W3c` à committer) · ✅ `H2` (`0cc8437`) · ✅ `Z1` · 🔄 `Z2`→`Z4`, `K1`→`K3`, lots 6 et 7 à faire |
...
> Décompte du plan T1 : **42 tâches actives** depuis la révision du 2026-09-22 — `H1` et `H2` ajoutées
> (35), puis le lot 4 bis `Z1` → `Z4` le même jour (`ADR-015`), puis `W2b` (2026-09-22), `W3b` et **`W3c`** (2026-09-23, points 40 et 41) —, 5 différées. `W3b` reste comptée : faite et commitée, puis remplacée par `W3c`, elle n'est pas retirée du plan. Corrigé une première fois le 2026-09-14 à 33 (le récapitulatif initial disait 31 — il
...
| # | Sujet | Nature |
|---|---|---|
| 1 | **Stockage local** — **préférence simple tranchée** ([`ADR-011`](adr/ADR-011-stockage-local.md), arbitrage du commanditaire du 2026-09-18) ; **moteur structuré non tranché** | `shared_preferences` **2.5.5** (publiée le 2026-03-25, BSD-3-Clause, éditeur `flutter.dev`, Windows couvert — relevé sur pub.dev le 2026-09-18 à 09:00 UTC). ✅ **Dans `pubspec.yaml` depuis `W1`** (`6b9e9ed`, 2026-09-22), résolue 2.5.5. Pour la donnée structurée (favoris, dernière vue), `drift` reste candidat par défaut ; `sqflite` seul **ne couvre pas Windows** — à trancher quand un écran en aura besoin |
...
| 21 | **`NV-W6` — chaque cran de molette déclenche un rechargement, sans anti-rebond.** `MapEventScrollWheelZoom` n'a pas de variante `…End` dans `flutter_map` 8.3.2 : un zoom de cinq crans fait cinq allers-retours au dépôt et cinq reconstructions des 4 150 marqueurs | Ouvert le 2026-09-13, **non mesuré**. À instruire avec `NFR-01` et `NV-W3` ([`nfr.md`](nfr.md)) · 🔄 Une fois `Z4` livrée (`ADR-015`, arbitré le 2026-09-22), sous le zoom 9 la carte ne dessinera plus que des pastilles de zone : les 4 150 reconstructions ne vaudront plus qu'au niveau individuel |
...
| 24 | **La PR de `feat/t1-mvvm-fiche-station` n'est pas ouverte** ; la branche n'est pas fusionnée sur `dev` | À ouvrir **par le commanditaire** — `gh` est absent du bac à sable (ligne 12) · **Branche poussée sur `origin` le 2026-09-18** à la demande du commanditaire ; la PR reste à ouvrir par lui · ✅ **PR #12 ouverte et fusionnée sur `dev`** (`5f02d3e`, 2026-09-23), branche supprimée |
...
| 30 | **Tap arbitraire entre marqueurs superposés** — au zoom national, 4 150 zones de tap de 44 pt se chevauchent ; `flutter_map` 8.3.2 empile les marqueurs dans l'ordre de la liste et le hit-test rend le plus tardif de l'asset, pas le plus proche du doigt. `04-ui.md § 3` demande un regroupement des zones qui se chevauchent ; `F2` non tranchée, `F2c` sans regroupement retenue par défaut | **Écart assumé pour T1, acté dans le plan (`U1`)** — à confirmer par le commanditaire · ✅ **Confirmé par le commanditaire le 2026-09-18** : écart accepté pour T1, à reprendre avec `X3` et `F2` · **Repris le 2026-09-22 par [`ADR-015`](adr/ADR-015-regroupement-par-zone-administrative.md)** (arbitrage du commanditaire) : sous le zoom 9, une pastille par région puis par département remplace les marqueurs superposés — ✅ **résolu sous le zoom 9 : `Z4` constatée à l'écran le 2026-09-23**, à l'écart près du point 43 (Île-de-France au niveau département). Au-delà du zoom 9, l'écart accepté le 2026-09-18 demeure là où des stations restent proches |
...
| 36 | **Constat d'écran du lot 3 (2026-09-22, Windows, `flutter run`, deux captures du commanditaire)** — **vu :** fond IGN et attribution, bascule des deux puces (marqueurs et légende changent ensemble), points ONDE sous leurs quatre formes plus le cercle pointillé, légende « débit » avec « ◇ ? Indéterminé » et sa phrase `BR-004`, aucun bandeau rouge. **Vu aussi :** « Non observé » et « Non renseigné » portent le même cercle pointillé (écart assumé, `onde_marker.dart`) ; la quasi-totalité des losanges sont à l'état `NonChargee`, légendé « Chargement en cours » alors que rien ne charge. **Vu aussi :** au zoom national, les marqueurs se recouvrent en une nappe illisible (constat du commanditaire, 2026-09-22), d'où l'arbitrage d'[`ADR-015`](adr/ADR-015-regroupement-par-zone-administrative.md) | **Libellé arbitré le 2026-09-22 : « Sélectionnez pour charger »** (verbe neutre souris, clavier, toucher ; conforme à `BR-014`, qui vise un usage de l'eau), appliqué dans `map_legend.dart`. ✅ **Complété le 2026-09-22 par le commanditaire, quatre sur quatre :** libellé « Sélectionnez pour charger » affiché, sélection d'une station → fiche, sélection d'un point ONDE → fiche, avis sur emprise déserte. **Le lot 3 est constaté à l'écran sur Windows** |
...
| 39 | **Fiche sans aucune mesure (station) ou sans campagne (ONDE) : pas d'encart daté** — l'encart de `04-ui.md § 5` porte « la date de la mesure ou de la campagne » ; sans date, il n'y a rien à dater | ✅ **Arbitré le 2026-09-23 par le commanditaire** : aucun encart, la phrase d'absence le dit déjà (`UC-003 A3`, `UC-004`) et le bandeau de la carte reste visible ; verrouillé par test (`W4`) · **Reformulé le 2026-09-23 par le point 41 (`W3c`)** : le contrôle « ⚠ Avertissement » est **toujours** en tête de fiche ; sans date, sa fenêtre n'a que le texte général, **sans phrase propre** — « sans date, pas de phrase datée », jamais une phrase inventée |
| 40 | **Bandeau de la carte : permanent ou fermable ?** — `W3` le posait permanent, sans aucun moyen de le fermer (`04-ui.md § 4`). Constat du commanditaire le 2026-09-23 : il veut le voir au départ, puis le retrouver par un menu | ✅ **Arbitré le 2026-09-23 par le commanditaire** : affiché à **chaque lancement**, fermable **pour la session** (rien n'est enregistré), rappelable par « Menu » › « Avertissement ». Écartés : premier lancement seulement, icône permanente réduite. `04-ui.md § 4-5` amendés, tâche `W3b`. Le constat « le bandeau disparaît après Ce que ça dit » n'a pas été reproduit en test (six variantes, tuiles réellement peintes) : cause non établie, sans objet depuis `W3b` · ⚠️ **Remplacé par le point 41 le même jour (2026-09-23)** : ni bandeau ni menu |
| 41 | **« Trop de bandeaux »** — constat du commanditaire le 2026-09-23 : le bandeau de carte (« Données indicatives. Ni autorisation, ni garantie. », rendu fermable et rappelable par le menu ☰ par `W3b` le même jour) **et** l'encart daté en tête des fiches (`W4`, « Mesure brute du… » / « Observation du… ») font deux bandeaux de trop | ✅ **Arbitré le 2026-09-23 par le commanditaire** — remplace l'arbitrage du point 40 : les deux sont **remplacés par un seul contrôle « ⚠ Avertissement »**, sur la carte en haut à droite **au-dessus de la légende** (le menu ☰ est retiré : une seule entrée, doublon) et **en tête de chaque fiche**, avant la valeur ou la catégorie. Il ouvre une fenêtre « Des informations, pas une autorisation » — titre et texte du modal initial, lecture seule, « Fermer » ; sur une fiche, la fenêtre est complétée **sous** le texte général par la phrase propre datée (« Mesure brute du … La station ne voit pas les lâchers de barrage. » / « Observation du … Ce n'est pas une mesure de débit… ») ; **sans date, pas de phrase propre**. **Inchangés :** l'écran du premier lancement (`BR-012`, `UC-006`) ; la date et la source à côté de chaque valeur (`BR-001`). **Écartés par le commanditaire :** garder le menu en plus du contrôle ; une fenêtre identique partout (les phrases propres des fiches seraient perdues). Tâche `W3c` ; `04-ui.md § 1, § 4, § 5`, `UC-001`, `UC-003`, `UC-004`, `UC-006` amendés. Code : `lib/features/shared/warning_link.dart` (`WarningLink`, `WarningWindow`) ; `map_warning_banner.dart`, `map_menu.dart`, `sheet_warning_card.dart` supprimés. ✅ **Constaté à l'écran sur Windows le 2026-09-23** par le commanditaire : aucun bandeau, contrôle « ⚠ Avertissement » présent, fenêtre conforme |
| 42 | 🚨 **54 stations métropolitaines aux coordonnées inversées dans le référentiel Hub'Eau** (`C-18`) — sous `code_projection 31`, `latitude_station`/`longitude_station` sont permutées (vérifié par appel réel le 2026-09-23, HTTP 200, `H000000201`, `H004000101`) ; `coordonnee_x/y_station` sont justes. Vu à l'écran : pastilles Hauts-de-France et Centre-Val de Loire déplacées | ✅ **Arbitré le 2026-09-23 par le commanditaire : lire X/Y pour ce seul code** — `5dde8d5`, l'asset reste la capture brute ; revu à l'écran le même jour. **Reste ouvert :** `J543211003` (Morbihan, projection 26, latitude 35,1) n'est pas une inversion, aucune correction inventée ; listée nommément dans `stations_asset_test.dart` |
| 43 | **Pastilles qui se chevauchent au niveau département** (constat de `Z4`, 2026-09-23) — Île-de-France (75, 92, 93, 94), et marqueurs corses sans rattachement dessinés sur la pastille 62 ; contraire à `04-ui.md § 3` (« regroupés automatiquement ») | ✅ **Écart accepté pour T1 par le commanditaire le 2026-09-23** (consigné dans `04-ui.md § 3`) : la sélection zoome sur la zone. À reprendre avec `X3`, sur constat. Écartés : fusion des pastilles proches (révise `ADR-015`), seuil de 9 abaissé |
| 44 | **Le contrôle « ⚠ Avertissement » est tronqué** en haut à droite (« Avertissemen… ») sur les captures du 2026-09-23, fenêtre ≈ 1 266 px, sous le ruban DEBUG | 🔄 À vérifier avec `K3` (taille minimale de fenêtre) — un avertissement ne se tronque jamais (`04-ui.md § 3`, `BR-012`) · ✅ **Écarté par test le 2026-09-23 (`K3`)** : la mise en page ne tronque pas le libellé à 800 × 700 ni à 1 266 × 713 ; « Avertissemen… » est le ruban DEBUG peint par-dessus (test « point 44 — ruban DEBUG »), absent d'un build release. Reste à le constater à l'écran en release (étape 6 de `K3`) |
| 45 | **À 800 × 400, la légende de l'échelle écoulement (six lignes) recouvre la colonne des contrôles de zoom** — constaté par test le 2026-09-23 (`K1`, `map_view_test.dart`, test « CONSTAT »), sous la taille minimale de fenêtre proposée (800 × 600) | 🔄 À trancher avec `K3` : la taille minimale de fenêtre l'écarte, ou la disposition change. Le test verrouille le fait, pas un invariant désiré · ✅ **Écarté le 2026-09-23 par la taille minimale de zone cliente 800 × 700** (décision 8 amendée par le commanditaire) : à 800 × 600, l'échelle débit recouvrait elle aussi les contrôles (test « RAISON DE L'AMENDEMENT »). Les faits à 800 × 400 et 800 × 600 restent verrouillés comme constats ; disposition inchangée. Import statique d'`AdjustWindowRectExForDpi` accepté : exclut seulement Windows 10 1507/1511 |
| 46 | **Coût du parcours au Tab dans la carte** (`K2`) — au zoom 9 en zone dense, 450 à 1 500 marqueurs dessinés selon la fenêtre, un arrêt de focus chacun ; en test (mode debug), ≈ 100 ms par appui et jusqu'à ≈ 900 appuis pour atteindre « ⚠ Avertissement » | 🔄 Conforme à l'arbitrage du 2026-09-23 ; **à mesurer en release avec `X3`** (`NFR-01`) |
| 47 | **Clés de marqueurs en double au-delà d'une fenêtre de 4 096 px** (`K2`) — au zoom minimal 4, une fenêtre plus large que le monde fait dessiner deux fois un même marqueur par `MarkerLayer`, avec la même `ValueKey` → erreur de clé dupliquée | 🔄 Cas marginal (écran > 4K), non traité : borner le zoom minimal ou contraindre la caméra le fermerait. Relevé par la relecture du 2026-09-23 |
...
~~Lot 4 : `W2`→`W5` (`W1` fait, `6b9e9ed`) ; puis `H2`, lot 4 bis `Z2`→`Z4` (`ADR-015`), `K1`→`K3`.~~ **Au 2026-09-23 :** lot 4 fait (`W1`→`W5`, `W2b`, `W3b`, `H1`) et `H2` fait (`0cc8437`) ; **`W3c`** (point 41) faite (`80a2d4b`) et ✅ **constatée à l'écran le 2026-09-23 : le lot 4 est clos**, PR #12 fusionnée sur `dev` (`5f02d3e`). Ensuite lot 4 bis `Z2`→`Z4` (`ADR-015`), `K1`→`K3`. **Et, depuis la levée du différé Android du 2026-09-18, un constat d'écran Android est dû : `flutter run -d emulator-5554`** — l'émulateur `Pixel_7` tourne, mais l'app n'a jamais été construite ni lancée sur Android sous Flutter ; c'est ce lancement qui fera passer `A⏸2` de 🔄 à ✅. Puis lots 6 et 7, dont `X5`, la purge React Native (ADR remplacés gardés, arbitré le 2026-09-18). Le fichier de poste `CLAUDE.local.md` porte le brief de reprise de cette session ; il est ignoré par git et à réécrire une fois la reprise faite.

⚠️ **Points encore à trancher par le commanditaire** : 27 (axe de remplissage quand les percentiles arriveront), 22 (caches résiduels sous `android/` du poste, à supprimer **par lui** — le gabarit, lui, est versionné depuis le 2026-09-18), 14 (garder la forme garantie de `D5` — recommandé, l'appel groupé ne garantit pas une mesure par station), 3 et 5 bis (périmètre). **Arbitrés le 2026-09-18** : 1, 16, 20-3, 23, 28, 29, 30, 34. **Arbitrés le 2026-09-22** : 19 (heure locale sans suffixe, via `H1`), le report de `BR-013` en T2, et le regroupement par zone administrative (`ADR-015`, reprend le point 30 sous le zoom 9).
