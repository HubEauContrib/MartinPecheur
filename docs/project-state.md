...
**Mis à jour :** 2026-09-23 — **« trop de bandeaux »** : bandeau de carte, menu et encart daté de fiche remplacés par un seul contrôle « ⚠ Avertissement » (point 41, tâche `W3c`, 42 tâches actives). Précédemment, 2026-09-22 — plan T1 révisé en place (`H1`, `H2` insérées, `BR-013` reporté en T2, heure locale arbitrée), puis **regroupement par zone administrative** arbitré (`ADR-015`, lot 4 bis `Z1`→`Z4`, 39 tâches actives). Précédemment, 2026-09-18 — **lot 3 de T1 clos** (`U1`→`U6`, les vues), après les lots 1 et 2 et le correctif `T-14`. Session interrompue à la demande du commanditaire (arrêt du poste) : voir § « Prochaine étape ».
...
| 41 | **« Trop de bandeaux »** — constat du commanditaire le 2026-09-23 : le bandeau de carte (« Données indicatives. Ni autorisation, ni garantie. », rendu fermable et rappelable par le menu ☰ par `W3b` le même jour) **et** l'encart daté en tête des fiches (`W4`, « Mesure brute du… » / « Observation du… ») font deux bandeaux de trop | ✅ **Arbitré le 2026-09-23 par le commanditaire** — remplace l'arbitrage du point 40 : les deux sont **remplacés par un seul contrôle « ⚠ Avertissement »**, sur la carte en haut à droite **au-dessus de la légende** (le menu ☰ est retiré : une seule entrée, doublon) et **en tête de chaque fiche**, avant la valeur ou la catégorie. Il ouvre une fenêtre « Des informations, pas une autorisation » — titre et texte du modal initial, lecture seule, « Fermer » ; sur une fiche, la fenêtre est complétée **sous** le texte général par la phrase propre datée (« Mesure brute du … La station ne voit pas les lâchers de barrage. » / « Observation du … Ce n'est pas une mesure de débit… ») ; **sans date, pas de phrase propre**. **Inchangés :** l'écran du premier lancement (`BR-012`, `UC-006`) ; la date et la source à côté de chaque valeur (`BR-001`). **Écartés par le commanditaire :** garder le menu en plus du contrôle ; une fenêtre identique partout (les phrases propres des fiches seraient perdues). Tâche `W3c` ; `04-ui.md § 1, § 4, § 5`, `UC-001`, `UC-003`, `UC-004`, `UC-006` amendés. Code : `lib/features/shared/warning_link.dart` (`WarningLink`, `WarningWindow`) ; `map_warning_banner.dart`, `map_menu.dart`, `sheet_warning_card.dart` supprimés. 🔄 **Jamais constaté à l'écran** |
