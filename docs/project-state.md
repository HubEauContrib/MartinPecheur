...
**Mis à jour :** 2026-09-14, fin de session — **lot 3 de T1 clos** (`U1`→`U6`, les vues), après les lots 1 et 2 et le correctif `T-14`. Session interrompue à la demande du commanditaire (arrêt du poste) : voir § « Prochaine étape ».
...
| Lots 4 à 7 | Avertissements (`W1`-`W5`), clavier/souris (`K1`-`K3`), documentation (`X1`-`X5`, dont la **purge React Native** `X5`), porte `0.2.0` (`P1`, `P2`) | 🔄 à faire |
...
> Décompte du plan T1 **corrigé le 2026-09-14** : **33 tâches actives** (le récapitulatif initial disait
> 31 — il oubliait `D8` — et `X5` a été ajoutée), 5 différées.

### Tests — l'état exact sur ce poste

**764 tests, 763 verts sur ce poste** (`flutter test`, 2026-09-14 en fin de session, sortie `+763 -1`). Jalons du jour : 427 à la reprise, 408 après le retrait YAGNI, 437 après `V2`, 452 après `V3`, 478 après le correctif `T-14`, 493 après `V4`, 540 après `U1`, 577 après `U2`, 651 après `U3`, 691 après `U4`, 711 après `U5`, 764 après `U6`. Le seul rouge
...
| 19 | **Le fuseau affiché est « UTC »** dans `stalenessNotice` (`JJ/MM/AAAA à HH:MM UTC`) | `U1` a **gardé UTC explicite** (`JJ/MM/AAAA à HH:MM UTC`) pour rester déterministe et cohérent avec `V1` ; l'heure locale reste une question fermée au commanditaire, non posée |
...
| 32 | **`BR-001` « … et sa source, visibles au même endroit »** : aucune fiche ne nomme Hub'Eau à côté d'une valeur ; `W4` (encart de fiche) ne le prévoit pas non plus | Lacune antérieure à T1, à porter dans `W4` ou dans un amendement de `BR-001` (`harold`) |
...
**Lot 4 — les quatre avertissements** (`W1` à `W5`), après deux questions fermées au commanditaire : (1) l'ajout de `shared_preferences` pour `W1` (version, licence, plateformes et date à relever sur pub.dev le jour même — `T-11` datait du 2026-09-13), (2) l'emplacement de l'encart partagé de `W4` (point 34). Avant tout : **`flutter run -d windows` par le commanditaire** — rien du lot 3 n'a été vu à l'écran (feuille au tap, bascule d'échelle, points ONDE, fiche ONDE, avis d'absence). Puis lots 5 à 7, dont `X5`, la purge React Native (question fermée sur les ADR remplacés : les garder). Le fichier de poste `CLAUDE.local.md` porte le brief de reprise de cette session ; il est ignoré par git et à réécrire une fois la reprise faite.

⚠️ **Points à trancher par le commanditaire, dans l'ordre où ils bloquent** : 34 (encart partagé, avant `W4`), 1 et 20-3 (`shared_preferences`, avant `W1`), 30 (tap superposé), 28 (« Non renseigné »), 29 (phrase de repli sur l'échelle débit), 27 (axe de remplissage quand les percentiles arriveront), 22-23 (dossier `android/` du poste, PNG Expo), 24 (PR de la branche).
