...
**Lot 4 — les quatre avertissements** (`W1` à `W5`) : **plus aucune question ne le bloque** depuis le 2026-09-18 (`ADR-011` écrit, `lib/features/shared/` arbitré et verrouillé). Avant tout : **`flutter run -d windows` par le commanditaire** — rien du lot 3 n'a été vu à l'écran (feuille au tap, bascule d'échelle, points ONDE, fiche ONDE, avis d'absence). Puis lots 5 à 7, dont `X5`, la purge React Native (question fermée sur les ADR remplacés : les garder). Le fichier de poste `CLAUDE.local.md` porte le brief de reprise de cette session ; il est ignoré par git et à réécrire une fois la reprise faite.

⚠️ **Points encore à trancher par le commanditaire** : 27 (axe de remplissage quand les percentiles arriveront), 22 (dossier `android/` du poste, à supprimer **par lui**), 24 (ouvrir la PR), 14 (garder la forme garantie de `D5` — recommandé, l'appel groupé ne garantit pas une mesure par station), 19 (UTC ou heure locale), 3 et 5 bis (périmètre). **Arbitrés le 2026-09-18** : 1, 16, 20-3, 23, 28, 29, 30, 34.
