...
| `NV-W5` — ~~Android ⏸ différé le 2026-09-12~~ **amendé le 2026-09-18 : le différé est levé** (arbitrage du commanditaire, amendement d'[`ADR-013`](adr/ADR-013-bascule-flutter-cible-windows.md)). Le constat se resserre : **l'application n'a jamais été construite ni lancée sur Android sous Flutter** | **ouvert, sans date.** Constaté le 2026-09-18 : `flutter doctor` voit la chaîne Android (SDK 36.0.0), le NDK exigé par Flutter 3.47.4 est présent sur le poste, le gabarit `android/` est généré, l'émulateur `Pixel_7` a démarré (`emulator-5554 device`, `sys.boot_completed=1`). **Non constaté :** aucune construction, aucun lancement, aucun écran — le bac à sable ne compile pas de natif ; `flutter run -d emulator-5554` revient au commanditaire. Aucune mesure de `NFR-01` sur Android non plus, et `NV-5` (appareil réel) reste ouvert |
