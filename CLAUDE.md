...
| **T1** | Carte, fiches, modal, bandeau et encart daté — encart renforcé (`BR-013`) en T2 | 🔄 en cours — plan révisé le 2026-09-22 (**41 tâches actives**, `H1`, `H2`, `W2b` et `W3b` insérées, heure affichée en heure locale sans suffixe, lot 4 bis `Z1`→`Z4` de regroupement par zone administrative, `ADR-015`) ; réusinage MVVM clos et relu ; lot 1 (données : fixtures ONDE, domaine, mapper, URI, dépôts hydro et ONDE, caches) clos ; lots 2 (`V1`→`V4`) et 3 (`U1`→`U6`, les vues) clos le 2026-09-14, **lot 3 constaté à l'écran sur Windows le 2026-09-22** ; lot 4 ouvert, `W1` fait (`6b9e9ed`) ; restent la fin du lot 4, le lot 4 bis et les lots 5 à 7 (dont `X5`, purge React Native en fin de T1) |
