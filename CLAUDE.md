...
|---|---|---|
| **T0** | Socle React Native + carte MapLibre + socle domaine + outillage percentiles | 🔄 **21 tâches sur 23 au 2026-08-15** — `docs/superpowers/plans/2026-07-31-t0-socle-react-native.md`. Lots 0 (hors `S5`), 1, 2 et 4 ✅ · `M1`–`M3` ✅ · 🚨 **`M4` exécutée, résultat négatif — `createPack` plante en natif, `ADR-012`** · restent `S5` et `M5`, tous deux en attente d'un **appareil Android réel** |
...
| Carte | **`@maplibre/maplibre-react-native` v11+** — MapLibre **Native**, rendu GPU. Fond **IGN Géoplateforme** (WMTS) | ✅ **`11.3.6` installée et compilée le 2026-08-15** — l'APK démarre sur émulateur (`M1`). Le fond IGN **s'affiche** (`M2`, `NV-2` levé le 2026-08-15). ⚠️ **v11 a changé l'API hors-ligne** : id auto-généré, `addListener`/`removeListener`. **Et bien plus encore** — `ShapeSource`/`CircleLayer`/`SymbolLayer` n'existent plus (`GeoJSONSource` + `Layer`), `clusterMaxZoomLevel` → `clusterMaxZoom`, `style` → `paint`/`layout`. **Lire les typings avant d'écrire : le plan T0 décrit la v10 et s'est trompé neuf fois** |
| Hors-ligne carto | **`OfflineManager.createPack`** — région + niveaux de zoom | 🚨 **EXÉCUTÉ le 2026-08-15, et il plante.** `SIGABRT` sur une `std::regex_error` non rattrapée, fil `DatabaseFileSource`, ~0,7 s après la création du pack. **4 essais sur 4**, base vierge comprise, et **avec le style vectoriel de démonstration de MapLibre** — ni l'IGN ni le raster en cause. `11.3.6` est la dernière version publiée. `NV-1` **ni confirmé ni infirmé** : le plantage précède le téléchargement de la première tuile. Arbitrage : `ADR-012` |
...
- **Release Android :** `npm run verify` → `npx expo prebuild --platform android --clean` → **`export ANDROID_HOME="$LOCALAPPDATA/Android/Sdk"`** → `cd android && ./gradlew assembleRelease` → `gh release create vX.Y.Z-alpha.N <apk> --prerelease`. Procédure, pièges et verrou produit : `docs/guide-release.md`. ✅ **Build exécuté le 2026-08-15** (110 Mo, 3 min 51) · 🔄 **publication jamais faite**. Trois pièges constatés : sans le `export`, Gradle prend le SDK Visual Studio et échoue sur `build-tools;35.0.0` · un tube (`| tee`) **masque le code de sortie de Gradle** · l'APK embarque **quatre ABI**, dont 52,5 Mo d'`x86` inutiles sur un téléphone
- **Signature de release :** le gabarit Expo signe `release` avec la **clé de debug publique** d'Android. `plugins/withReleaseSigning.js` (config plugin, déclaré dans `app.json`) câble un `signingConfigs.release` piloté par les propriétés `MARTINPECHEUR_*` ; la CI refuse de publier si `apksigner` lit encore `CN=Android Debug`. ✅ **plugin testé** (transformation de texte, sur une copie réelle du `build.gradle` généré) · 🔄 **jamais éprouvé par un build natif signé**, et la clé reste à générer
